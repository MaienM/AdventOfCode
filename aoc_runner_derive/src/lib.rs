...
/// Define a static that will hold a list of all [`aoc_runner::derived::Bin`]s for all binaries.
///
...
/// Define a static that will hold the [`aoc_runner::derived::Bin`] for the current file.
#[proc_macro_attribute]
...
///
/// The leading and trailing newline + a static amount of indentation for each line will be stripped to make the input match the original. The result will be stored in an [`aoc_runner::derived::Example`] along with the expected outputs (if provided).
///
...
/// Derive macro for [`aoc_runner::visual::ToRenderable`].
///
